#!/usr/bin/env python3
"""
Name of Service: Paper Trading Diagnostic Tool
Filename: paper_trading_diagnostic.py
Version: 1.0.0
Last Updated: 2025-01-27
REVISION HISTORY:
v1.0.0 (2025-01-27) - Initial diagnostic tool for paper trading troubleshooting
  - Checks service health and Alpaca connection status
  - Analyzes database trades for real vs simulated execution
  - Verifies Alpaca credentials configuration
  - Reviews trading signals and execution errors
  - Supports running from Testing folder or main directory

DESCRIPTION:
This diagnostic tool provides comprehensive analysis of the paper trading system
to determine if trades are being executed through Alpaca Paper Trading API or
running in simulation mode. It helps identify configuration issues and provides
...
class PaperTradingDiagnostic:
    def __init__(self):
        ...
        # Check if we're in a subdirectory (like Testing)
        if os.path.exists('../trading_system.db'):
            self.db_path = '../trading_system.db'
            self.log_path = '../logs/paper_trading_service.log'
    ...
    def check_alpaca_credentials(self):
        ...
        # Check .env file in parent directory if running from Testing folder
        env_paths = []
        if os.path.exists('../trading_system.db'):
    ...
    def check_trade_errors(self):
        ...
        if os.path.exists(self.log_path):
            print(f"Checking {self.log_path} for errors...")
            ...
            print(f"✗ Paper trading log file not found at: {self.log_path}")
